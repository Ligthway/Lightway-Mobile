...
struct FallingParticleWithRectangle {
...
struct FallingParticleWithRectangleUniforms {
    var deltaTime: Float
...
class FallingParticleWithRectangleRenderer {
    ...
    init?(device: MTLDevice) {
        ...
        guard let updateFunction = library.makeFunction(name: "updateParticles"),
              let spawnFunction = library.makeFunction(name: "spawnParticles") else {
            return nil
        }
        ...
        // Create buffers
        let particleBufferSize = maxParticles * MemoryLayout<FallingParticleWithRectangle>.stride
        guard let particleBuffer = device.makeBuffer(length: particleBufferSize, options: .storageModeShared),
              let uniformsBuffer = device.makeBuffer(length: MemoryLayout<FallingParticleWithRectangleUniforms>.stride, options: .storageModeShared),
    ...
    func update(deltaTime: Float, screenWidth: Float, screenHeight: Float, particleAreaHeight: Float) {
        ...
        // Update uniforms
        let uniformsPtr = uniformsBuffer.contents().bindMemory(to: FallingParticleWithRectangleUniforms.self, capacity: 1)
        uniformsPtr.pointee = FallingParticleWithRectangleUniforms(
            deltaTime: deltaTime,
            screenWidth: screenWidth,
            screenHeight: screenHeight,
            spawnRate: 150.0, // particles per second (reduced to lower density)
        ...
        if let computeEncoder = commandBuffer.makeComputeCommandEncoder() {
            ...
            let particlesToSpawn = Int(250.0 * deltaTime) + 1
            let threadsPerThreadgroup = MTLSize(width: 64, height: 1, depth: 1)
            let threadgroupsPerGrid = MTLSize(
